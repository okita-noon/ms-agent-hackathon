...
> 最終更新: 2026-05-21（UIすり合わせ反映：受注ステータス整理・納品グループ・メモ欄・分析タブ分離・LINE返信簡潔化・ピッキングスコープ外明記）
...
UIすり合わせの結果、ハッカソン提出までは下記をスコープ外とし、営業担当向け機能（受注一覧・分析・顧客管理・Dashboard Agent）に注力する。

| タスク | スコープ外とした理由 |
|---|---|
| ピッキングリストPDF生成 | 営業担当向け機能を優先するため。`src/models/picking.py` のモデル定義は将来の拡張のために残す（#68） |
