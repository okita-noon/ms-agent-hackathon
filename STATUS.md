...
> 最終更新: 2026-05-21（イシュー #63/#64/#65/#66/#67/#68/#69 対応）
...
- [x] ダッシュボード（React + Vite + Tailwind）
  ...
  - 顧客一覧（常設編集ボタン・納品グループ表示）
  - プロフィールドロップダウン（ログアウト）
  - API未接続時のデモデータフォールバック
  - 分析タブ（ステータス別・チャネル別ドーナツチャート・統計カード、注文一覧から分離）
  - 受注詳細モーダルのメモ欄（担当者コメント・申し送り、インライン編集対応）
...
| ~~ピッキングリストPDF生成~~ | ~~2h~~ | **スコープ外**（#68）。営業担当機能に注力するため今回のハッカソン提出には含めない |
