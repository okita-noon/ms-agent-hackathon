...
- [x] 既存受注を別セッションから変更した場合、元の注文会話 `session_id` を保持し、変更時のセッションを `related_session_ids` に追加。受注詳細では変更前・変更後の電話/LINE/メールログを時系列で表示
...
- [x] ダッシュボード（React + Vite + Tailwind）
  ...
  - 受注一覧上部の「確認が必要」件数を、ステータス `要対応` の総数ではなくモーダルと同じAI検知済み注文数に統一
