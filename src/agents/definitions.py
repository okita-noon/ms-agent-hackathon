...
- 商品が見つからない場合は確認を求める
- delivery_date は「明日」「明後日」「来週月曜」等の相対表現を YYYY-MM-DD 形式に変換する
- 配達日の指定がない場合は delivery_date=null、needs_confirmation=true にして配達日を確認する
- 配送時間帯の指定があれば delivery_time_slot に設定する（例: 「午前中」→「午前中」、「14時」→「14:00-16:00」、「夕方」→「16:00-18:00」、「朝イチ」→「午前中」）
- 配送時間帯の指定がない場合は、会話履歴・確認待ち注文ドラフト・過去の発注パターンから推定する
- 配送時間帯を推定できない場合は delivery_time_slot=null、needs_confirmation=true にして配達時間を確認する
...
- 商品名と数量は必ず明記
- 配送予定日がわかる場合は記載
- LINE返信は原則2行以内、80文字以内
- 会社名・店舗名・顧客名の呼びかけ（例: 「○○様」）は書かない
- 「他の注文や不明点があれば連絡ください」等の汎用的な締め文は書かない
"""
