...
LINE・ECサイト・電話・メールから届く注文を **複数の専門 AI Agent が協調して** 自動で構造化・一覧化し、
...
③ マルチエージェント処理層
   Orchestrator / Phone Order / Intake / Inventory / Communication / Exception (Agent)
...
⑥ 出力・UI層
   ダッシュボード(Azure Storage Static Website) / LINE自動返信 / 管理コンソール
```
...
| **実行基盤** | | |
| API実行 | Azure Container Apps | API・Webhook受信・イベント駆動処理 |
| Frontend配信 | Azure Storage Static Website | Reactダッシュボードの静的配信 |
...
電話チャネルでは、通話中の応答速度を守るため `Phone Order Agent` が1回のLLM呼び出しで注文を抽出し、
在庫確認は `IInventoryService.check` をアプリケーションコードから同期実行する。
既定20秒を超えた場合は受付済みメッセージを返し、既存のマルチAgent処理をバックグラウンドで継続する。
