...
顧客 → LINE メッセージ送信
    ...
    → Orchestrator Agent (Semantic Kernel)
...
顧客 → 電話発信
    ...
    → Container Apps
       ├→ IntentUnderstandingService（雑談・挨拶 / 注文照会 / 在庫照会 / 注文を分岐）
       ├→ Phone Order Agent（注文抽出、電話中の同期応答）
       ├→ IInventoryService.check（Connector経由で同期在庫確認）
       ├→ 在庫OKなら同期ルート内で Cosmos DB に受注保存・在庫引当
       └→ TTSで在庫確認結果と受付完了を返答（受注Noは読み上げない。既定20秒でフォールバック）
    → 担当者ダッシュボードに表示 + SMS通知
    → Learning Service (非同期)
```
...
Phone Order Agent による同期応答、在庫確認、Cosmos DB保存、Learning Service まで本番電話フローと同じ経路を通す。
...
在庫OKで顧客確認が不要な注文は同期ルート内で保存する。超過時や同期保存前に例外が起きた場合は
「受付済み・確認後登録」のTTSを返し、`PHONE_BACKGROUND_VALIDATION_ENABLED=true` の場合は裏で既存マルチAgent処理を継続する。
...
挨拶・天気の話・感謝だけのような雑談発話は注文抽出に渡さず、短い自然応答と注文促しを返す。
「今の注文は？」のような注文照会も LINE と同じ会話分岐で扱い、電話・メールでも未配送の受注概要を返す。
メールチャネルも `message-history` の直近履歴を Orchestrator に渡し、確認待ちドラフトや前回返信と矛盾しない返信生成に使う。
...

{
  ...
  "agent_thread_id": "thread_abc123",   // Semantic Kernel の会話スレッド ID
...
注文メッセージの入口では `IntentUnderstandingService` が Intent を分類する。明確な在庫照会・現在注文照会・全体キャンセルは deterministic rule で処理し、現在注文がある状態で曖昧な取消表現が来た場合は Orchestrator Agent に JSON intent 分類を依頼する。`いつもの` / `前と同じ` は `OrderMemoryService` で学習パターンまたは過去注文からドラフト復元し、`InventoryApplicationService` の在庫確認後に通常の受注確定フローへ流す。
...
別セッションの電話・LINE・メール発話で既存受注を変更した場合は、元の `session_id` は保持したまま
変更時のセッションを `related_session_ids` に追加し、変更前後の会話ログをどちらも参照できるようにする。

```
Order (Cosmos DB: order-documents)
  ├── session_id: "sess-U1234-20260515001"  ← セッションID（Optional）
  ├── related_session_ids: ["sess-U1234-20260515110000"]  ← 変更時などの追加セッション
  │
...
API:
  GET /api/orders/{order_id}/messages
    → Order.session_id と Order.related_session_ids を取得
    → IMessageHistoryRepository.list_by_session_id() で各セッションの会話取得
    → created_at で時系列に並び替え
